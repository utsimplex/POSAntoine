...
{
    public class GastosAdapter : Adapter
    {



        public void registrarGasto(ref Entidades.Gasto gasto)
        {
            // Crear conexión
            ...
            try
            {
                ...
                // Crear SqlCommand
                SqlCommand cmd = new SqlCommand("AddGasto", con);
                cmd.CommandType = CommandType.StoredProcedure;

                // Agregar los parámetros al comando
                cmd.Parameters.AddWithValue("@pFecha", gasto.Fecha.ToString("yyyyMMdd"));
                cmd.Parameters.AddWithValue("@pTipo", gasto.TipoGasto);
                cmd.Parameters.AddWithValue("@pMonto", gasto.Monto);
                cmd.Parameters.AddWithValue("@pDescripcion", gasto.Descripcion);
                cmd.Parameters.AddWithValue("@pCajaID", gasto.CajaId);
                cmd.Parameters.Add("@id", SqlDbType.Int).Direction = ParameterDirection.Output;

                // Ejecutar el stored procedure
                cmd.ExecuteNonQuery();
                int id = cmd.Parameters["@id"].Value != DBNull.Value? Convert.ToInt32(cmd.Parameters["@id"].Value):0;
                if (id!=0)
                gasto.id = (int)id;
            }
            catch (Exception ex)
        ...
        public List<Entidades.Gasto> GetAll()
        {
            List<Entidades.Gasto> ListaGastos = new List<Entidades.Gasto>();
            //Crear Conexion y Abrirla
            SqlConnection Con = CrearConexion();

            // Crear SQLCeCommand - Asignarle la conexion - Asignarle la instruccion SQL (consulta)
            SqlCommand Comando = new SqlCommand("SELECT [id],[fecha],[tipoGasto],[monto],[cajaId],[descripcion] FROM Gastos ", Con);
            try
            {
                Comando.Connection.Open();
                SqlDataReader drGastos = Comando.ExecuteReader();

                while (drGastos.Read())
                {
                    Entidades.Gasto gasto = new Entidades.Gasto();

                    gasto.id = (int)drGastos["id"];
                    gasto.Fecha= (DateTime)drGastos["fecha"];
                    gasto.TipoGasto = (string)drGastos["tipoGasto"];
                    gasto.Monto = (decimal)drGastos["monto"];
                    gasto.CajaId = drGastos["cajaId"] != DBNull.Value ? Convert.ToInt32(drGastos["cajaId"]) : (int?)null;
                    gasto.Descripcion = (string)drGastos["descripcion"];
        ...
        public List<Entidades.Gasto> GetAllEntreFechas(DateTime fechaDesde, DateTime fechaHasta)
        {
            ...
            // Crear SQLCeCommand - Asignarle la conexion - Asignarle la instruccion SQL (consulta)
            SqlCommand Comando = new SqlCommand("SELECT * FROM Gasto WHERE CAST(fecha AS DATE) BETWEEN @fechaDesde AND @fechaHasta;", Con);
        ...
        public List<Entidades.Gasto> GetAllFiltro(string filtro)
        {
            ...
            // Crear SQLCeCommand - Asignarle la conexion - Asignarle la instruccion SQL (consulta)
            SqlCommand Comando = new SqlCommand("SELECT * FROM Gasto WHERE TipoGasto like @filtro OR descripcion like @filtro ", Con);
            Comando.Parameters.Add(new SqlParameter("@filtro", SqlDbType.NVarChar));
            ...
            CREATE PROCEDURE [dbo].[AddGasto]
                ...
                @pCajaID INT
                    ...
                    BEGIN
                        ...
                        INSERT INTO Gastos (fecha,tipoGasto,monto,descripcion,cajaId)
                        VALUES (@pFecha, @pTipo, @pMonto, @pDescripcion, @pCajaID);
	                    SELECT @@IDENTITY
                    END
         */
