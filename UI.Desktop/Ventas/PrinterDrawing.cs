    ...
    public class PrinterDrawing
    {
        ...
        private void pd_PrintPage(object sender, PrintPageEventArgs ev)
        {
            ...
            if (_modo == "CLIENTE")
            {
                ...
                ev.Graphics.DrawString(_cabeceraComanda, headingFont, Brushes.Black, 138-(Cabecera.Width/2), height, new StringFormat());
                height += 30;
                //Print Direccion
                    SizeF Direccion = ev.Graphics.MeasureString(_direccionComanda, lessNormalFont);
                ev.Graphics.DrawString(_direccionComanda, lessNormalFont, Brushes.Black, 138 - (Direccion.Width / 2), height, new StringFormat());
                height += 20;
                ////Print Telefono
                    SizeF Telefono = ev.Graphics.MeasureString(_telefonoComanda, lessNormalFont);
                ev.Graphics.DrawString(_telefonoComanda, lessNormalFont, Brushes.Black, 138 - (Telefono.Width / 2), height, new StringFormat());
                height += 20;
                ...
                ev.Graphics.DrawString("Precio", boldFont, Brushes.Black, 150, height, new StringFormat());
                ev.Graphics.DrawString("Total", boldFont, Brushes.Black, 240, height, new StringFormat());
                height += 20;
                ...
                foreach (var item in _venta_actual_articulos)
                {
                    ...
                    ev.Graphics.DrawString(item.Precio.ToString("c"), comandaNormalFont, Brushes.Black, 155 + (50 - priceWidth.Width), height, new StringFormat());
                    ev.Graphics.DrawString((item.Cantidad * item.Precio).ToString("c"), comandaNormalFont, Brushes.Black, 240 + (50 - totalWidth.Width), height, new StringFormat());
                    if (item.Descuento != 0)
                    {
                        ...
                        ev.Graphics.DrawString((item.Descuento *-1).ToString("c"), comandaNormalFont, Brushes.Black, 240 + (50 - descuentoWidth.Width), height, new StringFormat());
                    }
                    else if(item.Descuento_porcentaje !=0)
                    {
                        SizeF descuentoWidth = ev.Graphics.MeasureString((item.Descuento_porcentaje / 100 * item.Cantidad * item.Precio * -1).ToString("c"), comandaNormalFont);
                        height += 15;
                        ev.Graphics.DrawString("Descuento", comandaNormalFont, Brushes.Black, 110 + qtyWidth.Width, height, new StringFormat());
                        ev.Graphics.DrawString((item.Descuento_porcentaje/100*item.Cantidad * item.Precio *-1).ToString("c"), comandaNormalFont, Brushes.Black, 240 + (50 - descuentoWidth.Width), height, new StringFormat());
                    }
                    height += 20;
            ...
            else if (_modo=="CAMBIO")
            {
                 ...
                 else if (_modo == "FISCAL")
                {
                    ...
                    foreach (var item in _venta_actual_articulos)
                    {
                        ...
                        SizeF totalWidth = ev.Graphics.MeasureString(item.Subtotal.ToString("c"), comandaNormalFont);
                        SizeF netTotalWidth = ev.Graphics.MeasureString((item.Precio * item.Cantidad).ToString("c"), comandaNormalFont);
                        ...
                        ev.Graphics.DrawString(item.DescripcionArticulo, comandaNormalFont, Brushes.Black, 10, height, new StringFormat());
                        height += 15;
                        ev.Graphics.DrawString(item.Cantidad.ToString(), comandaNormalFont, Brushes.Black, 60, height, new StringFormat());
                        if (_venta_Actual.letraComprobateRO == "A")
                        {
                        ev.Graphics.DrawString("$ " + (item.Precio).ToString("c"), comandaNormalFont, Brushes.Black, 170 - netPriceWidth.Width, height, new StringFormat());
                        ev.Graphics.DrawString("(21)", comandaNormalFont, Brushes.Black, 180, height, new StringFormat());
                        ev.Graphics.DrawString("$ " + (item.Precio * item.Cantidad).ToString("c"), comandaNormalFont, Brushes.Black, 275 - netTotalWidth.Width, height, new StringFormat());
                        height += 15;
                        }
                        else
                        {
                        ...
                        SizeF descuentoWidth = ev.Graphics.MeasureString((item.Descuento_porcentaje / 100 * item.Cantidad * item.Precio * -1).ToString("c"), comandaNormalFont);
                            ev.Graphics.DrawString("Descuento", comandaNormalFont, Brushes.Black, 110 + qtyWidth.Width, height, new StringFormat());
                            //ev.Graphics.DrawString("$ " + Convert.ToString(item.Precio), comandaNormalFont, Brushes.Black, 170 + (50 - priceWidth.Width), height, new StringFormat());
                            ev.Graphics.DrawString((item.Descuento_porcentaje / 100 * item.Cantidad * item.Precio *-1).ToString("c"), comandaNormalFont, Brushes.Black, 280 - descuentoWidth.Width, height, new StringFormat());
                            height += 15;
                        }
                    }
